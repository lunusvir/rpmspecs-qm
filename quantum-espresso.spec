...
%global extra_gfortran_flags -fallow-argument-mismatch -fno-lto -fPIC
%else
%global extra_gfortran_flags -fPIC
#%%{nil}
%endif
...
%global qe_version              7.2
%global qe_commit               7.2
%global d3q_commit              2c41405306b1aaecb8dcce2fe9f50631828152ec
%global devxlib_commit          0.1.0
%global fox_commit              4.1.2-QE
%global mbd_commit              82005cbb65bdf5d32ca021848eec8f19da956a77
%global pw2qmcpack_commit       f72ab25fa4ea755c1b4b230ae8074b47d5509c70
%global gipaw_commit            7.2
%global wannier90_commit        3.1.0
%global want_commit             2.6.1
%global yambo_commit            4.5.2
%global w90_path                W90
...
sed -i -e 's/wannier90-3.1.0/W90/' EPW/src/Makefile
...
for e in devxlib d3q fox mbd pw2qmcpack qe-gipaw wannier90 want yambo; do \
if [[ -e external/$e ]]; then rm -rf external/$e; fi \
mkdir -p external/$e \
done && \
for l in D3Q GIPAW W90 WANT YAMBO; do \
if [[ -L $l ]]; then rm -f $l \
...
tar -xf %{SOURCE11} -C external/yambo/lib/archive/ &&\
sed -i -e 's#$(COMPILER) \\([^ ]\\+\\.F90\\)#$(COMPILER) $(POSTOPTS) $(FCOPTS) \\1#' external/wannier90/src/Makefile.2 &&\
for f in $(grep -rl 'python$' external/wannier90/test-suite); do sed -i -e 's#bin/env python$#bin/env python3#' "$f"; done &&\
sed -i -e 's#SUBDIRS = src lapackified distributed thermal2#SUBDIRS = src minpack/lapackified minpack/distributed thermal2#' external/d3q/Makefile &&\
sed -i -e 's#dfftp%%l#dfftp%%nl#' external/d3q/src/efermi_shift.f90 &&\
...
if [ "$MPI_SUFFIX" == "_serial" ]; then FORTRAN='gfortran'; CONFIGURE='--disable-parallel'; export MPI_LIBS=""; export SCALAPACK_LIBS="-lscalapack"; fi&& \
...
if [ "$MPI_SUFFIX" != "_serial" ]; then FORTRAN='mpif90'; CONFIGURE='--enable-parallel --with-scalapack=yes'; export MPI_LIBS="-L${MPI_LIB} $LIBMPI"; export SCALAPACK_LIBS="-L${MPI_LIB} -lscalapack"; fi&& \
    export CC=gcc &&\
    export CXX=c++ &&\
    export F90="${FORTRAN}" &&\
...
%changelog
* Sat Nov 11 2023 L <lunusvir@gmail.com> - 7.2-3
- Update to 7.2
- Patch d3q and wannier90
- Increase gipaw - cgsolve_all - max iteration to 300
- Fix "relocation R_X86_64_32 against .rodata" by adding -fPIC to FFLAGS
